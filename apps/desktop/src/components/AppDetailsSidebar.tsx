      ...
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto p-0">
        ...
        <div className="px-4 pb-4 space-y-4">
          ...
          {isMcp && app.transport && app.transport.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Transport Types</h3>
            ...
            <div className="space-y-3">
              {isUrlBased && (
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs break-all">
                      {app.config.url}
                    </div>
                  </div>
                  {app.config.headers && Object.keys(app.config.headers).length > 0 && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground block mb-1">Headers</label>
                      <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs overflow-x-auto">
              ...
              {isCommandBased && (
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs">
                      {app.config.command}
                    </div>
                  </div>
                  {app.config.args && app.config.args.length > 0 && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground block mb-1">Arguments</label>
                      <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs">
                  ...
                  {app.config.env && Object.keys(app.config.env).length > 0 && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground block mb-1">Environment Variables</label>
                      <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs overflow-x-auto">
              ...
              {isApi && (
                <div className="space-y-3">
                  {app.config.url && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground block mb-1">API URL</label>
                      <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs break-all">
                        {app.config.url}
                      </div>
                    </div>
                  ...
                  {app.config.oas && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground block mb-1">OpenAPI Spec</label>
                      <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs break-all">
                        {app.config.oas}
                      </div>
                    </div>
              ...
              {!isUrlBased && !isCommandBased && !isApi && app.config && (
                <div>
                  <label className="text-xs font-medium text-muted-foreground block mb-1">Config</label>
                  <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs overflow-x-auto">
          ...
          {/* App ID */}
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide block mb-1.5">App ID</label>
            <div className="p-2.5 rounded-lg bg-muted/50 border font-mono text-xs">
              {app.id}
            </div>
          </div>
